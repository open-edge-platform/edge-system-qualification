...
def eval_cpu_usage(cpu_file):
    ...
    with open(cpu_file, "r") as f:
        ...
        if is_intel_xeon() and sock_num > 1:
            return 99.9 if one_socket_cpu_usage * 2 > 100.0 else one_socket_cpu_usage * 2
...
def _start_cpu_usage_thread(event, out_file):
    thread_cpu = threading.Thread(
        target=collect_usage, args=(["vmstat", "-w", "-t", "1"], out_file, event, cpu_usage_filter)
    )
    thread_cpu.start()
    return thread_cpu
...
def _start_mem_usage_thread(event, out_file):
    thread_mem = threading.Thread(
        target=collect_usage, args=(["free", "-w", "-s", "1"], out_file, event, mem_usage_filter)
    )
    thread_mem.start()
    return thread_mem
...
def _eval_gpu_telemetry(out_result, gpu_devs):
    out_result["Package Power"] = -1.0  # init
    gpu_freq_list = []
    gpu_power_list = []
    gpu_rcs_usage = []
    gpu_vcs_usage = []
