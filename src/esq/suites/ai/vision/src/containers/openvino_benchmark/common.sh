...
# iGPU Device IDs - Updated Feb 2026 with Panther Lake, Nova Lake, Lunar Lake, Arrow Lake, Meteor Lake
iGPU_Dev_IDs=(A7A9 A7A8 A7A1 A7A0 A721 A720 A78B A78A A789 A788 A783 A782 A781 A780 4907 4905 4680 4682 4688 468A 468B 4690 4692 4693 46D0 46D1 46D2 4626 4628 462A 46A0 46A1 46A2 46A3 46A6 46A8 46AA 46B0 46B1 46B2 46B3 46C0 46C1 46C2 46C3 4C8A 4C8B 4C90 4C9A 4C8C 4C80 4E71 4E61 4E57 4E55 4E51 4571 4557 4555 4551 4541 9A59 9A60 9A68 9A70 9A40 9A49 9A78 9AC0 9AC9 9AD9 9AF8 6420 64A0 64B0 7D51 7D67 7D41 7DD1 7DD5 7D45 7D40 7D55 0BD5 0BDA 56C0 56C1 A7AA A7AB A7AC A7AD 4908 4909 46D3 46D4 B080 B081 B082 B083 B084 B085 B086 B087 B08F B090 B0A0 B0B0 FD80 FD81 D740 D741 D742 D743 D744 D745)

# dGPU Device IDs - Updated Feb 2026 with Battlemage (Arc B-series) and additional Alchemist
dGPU_Dev_IDs=(56B3 56B2 56A4 56A3 5697 5696 5695 56B1 56B0 56A6 56A5 56A1 56A0 5694 5693 5692 5691 5690 56A2 56BA 56BB 56BC 56BD 56BE 56BF 56AF 56C0 56C1 56C2 E202 E209 E20B E20C E20D E210 E211 E212 E215 E216 E220 E221 E222 E223)
...
find_available_gpus()
{
  ...
  while IFS=" " read -r id_info; do
    ...
    #device_id=$(echo "$id_info" | sed -n 's/.*:\([^]]*\)\].*/\1/p')
  ...
  # Print the extracted vendor and device IDs
  #echo "Vendor IDs: ${vendor_ids[*]}"
  #echo "Device IDs: ${device_ids[*]}"
  #echo "Available GPU Devices: ${available_devices[*]}"
...
format_log()
{
	...
	echo -e "\n[$(eval "$time")][$baseFileName][$funcName][line:$lineNo]${body}" 2>&1 | tee -a "$detailsLogFile"
    elif [ "${returnCode}" -eq 0 ]; then
        echo -e "\n[$(eval "$time")][$baseFileName][$funcName][line:$lineNo]${_green}${body}${_reset}" 2>&1 | tee -a "$detailsLogFile"
    else
        echo -e "\n[$(eval "$time")][$baseFileName][$funcName][line:$lineNo]${_red}${body}${_reset}" 2>&1 | tee -a "$detailsLogFile"
    fi
}
