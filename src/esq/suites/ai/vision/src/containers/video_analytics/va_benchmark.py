...
class VABenchmark(BaseVideoAnalyticsBenchmark):
    ...
    def bpl_run_benchmark(self):
        ...
        for j, (mode, devices) in enumerate(self.config["modes"].items()):
            ...
            if is_concurrent_mode and not check_available_memory(min_available_gb=6.0, logger=self.logger):
                self.logger.warning(
                    f"[VA] Insufficient memory for concurrent mode {mode}, skipping this mode"
                )
                continue
