...
"""
Video Analytics (VA) Light Pipeline Test.

This module provides the Light weight video analytics pipeline benchmark:
- Detection: YOLOv11n (lightweight, fast inference)
...
def _download_va_light_resources(resources_dir: str) -> tuple:
    """
    Download VA Light pipeline resources.
    ...
    # Download VA Light models and videos using VA utility
    logger.info("Downloading VA Light pipeline resources (models and videos)...")
    try:
        from esq.utils.models.va_resources import download_va_resources

        results = download_va_resources(models_path, videos_path)
        if not results:
            raise RuntimeError("Failed to download VA Light resources - no results returned")

        logger.info("VA Light resources prepared:")
        logger.info(f"  Detection model: {results.get('detection_xml')}")
        logger.info(f"  Classification model: {results.get('classification_xml')}")
        ...
        logger.error(f"Failed to download VA Light resources: {e}")
        raise RuntimeError(f"VA Light resource download failed: {e}") from e
...
def test_va_light(
    ...
    """
    End-to-end Video Analytics Light Pipeline Benchmark using Docker container.
    ...
    test_display_name = configs.get("display_name", "VA Light Pipeline")
    force_run = configs.get("force_run", False)

    logger.info(f"Starting VA Light Pipeline Benchmark: {test_display_name} (force_run={force_run})")
    ...
    va_results = os.path.join(data_dir, "results", "va_light")
    ...
    try:
        ...
        def prepare_assets():
            ...
            # Download VA Light resources
            models_path, videos_path = _download_va_light_resources(resources_dir)
        ...
        prepare_test(test_name=test_name, configs=configs, prepare_func=prepare_assets, name="va_light_assets")
    ...
    csv_filename = f"va_light_pipeline_{test_id}.csv"
        ...
        def run_test():
            ...
            try:
                ...
                if should_run_benchmark:
                    if force_run:
                        logger.info("Force run enabled. Re-running VA Light benchmark for all devices...")
                        modes_to_run = expected_modes
                    else:
                        logger.info(f"Running VA Light benchmark for modes: {modes_to_run}")
                    ...
                    try:
                        ...
                        container_name = f"{configs.get('container_image', 'va_light_bm')}_light"
                        container_result = run_va_container(
                            docker_client=docker_client,
                            container_name=container_name,
                            image_name=configs.get("container_image", "va_light_bm"),
                            image_tag=configs.get("image_tag", "1.0"),
                            benchmark_script="va_benchmark",
    ...
    if results.metadata.get("status") == "N/A" and "failure_reason" in results.metadata:
        ...
        pytest.fail(f"VA Light test failed - {failure_msg}")
