...
"""
Video Analytics (VA) Medium Pipeline Test.

This module provides the Medium weight video analytics pipeline benchmark:
- Detection: YOLOv5m (medium model, 640x640)
- Classification: ResNet-50 + MobileNet-v2 (dual classification)
- Video: apple.mp4 (H265)

The VA Medium pipeline is more complex than Light:
- Uses larger detection model (YOLOv5m vs YOLO11n)
- Dual classification for richer feature extraction
- H265 video codec for more realistic workload

The pipeline supports different compute modes:
...
# CSV files for Medium pipeline (different from Light)
VA_MEDIUM_CSV_FILES = ["va_medium_pipeline.csv"]


def _download_va_medium_resources(resources_dir: str) -> tuple:
    """
    Download VA Medium pipeline resources.
    ...
    # Download VA Medium models and videos using VA utility
    logger.info("Downloading VA Medium pipeline resources (models and videos)...")
    try:
        from esq.utils.models.va_resources import download_va_medium_resources

        results = download_va_medium_resources(models_path, videos_path)
        if not results:
            raise RuntimeError("Failed to download VA Medium resources - no results returned")

        logger.info("VA Medium resources prepared:")
        logger.info(f"  Detection model: {results.get('detection_xml')}")
        logger.info(f"  Classification ResNet-50: {results.get('classification_resnet_xml')}")
        logger.info(f"  Classification MobileNet-v2: {results.get('classification_mobilenet_xml')}")
        ...
        logger.error(f"Failed to download VA Medium resources: {e}")
        raise RuntimeError(f"VA Medium resource download failed: {e}") from e
...
def test_va_medium(
    ...
    """
    End-to-end Video Analytics Medium Pipeline Benchmark using Docker container.

    Medium Pipeline:
    - Detection: YOLOv5m (INT8, 640x640)
    - Classification 1: ResNet-50 (INT8)
    - Classification 2: MobileNet-v2 (INT8)
    - Video: apple.mp4 (H265)
    ...
    Medium pipeline has lower expected throughput than Light due to:
    - Larger detection model (YOLOv5m)
    ...
    test_display_name = configs.get("display_name", "VA Medium Pipeline")
    force_run = configs.get("force_run", False)

    logger.info(f"Starting VA Medium Pipeline Benchmark: {test_display_name} (force_run={force_run})")

    dockerfile_name = configs.get("dockerfile_name", "Dockerfile")
    docker_image_tag = f"{configs.get('container_image', 'va_medium_bm')}:{configs.get('image_tag', '1.0')}"
    timeout = int(configs.get("timeout", 900))  # Medium needs more time
    ...
    va_results = os.path.join(data_dir, "results", "va_medium")
    ...
    try:
        ...
        def prepare_assets():
            ...
            # Download VA Medium resources
            models_path, videos_path = _download_va_medium_resources(resources_dir)
        ...
        prepare_test(test_name=test_name, configs=configs, prepare_func=prepare_assets, name="va_medium_assets")
    ...
    csv_filename = f"va_medium_pipeline_{test_id}.csv"
    ...
    try:
        ...
        def run_test():
            ...
            try:
                ...
                # Check existing results in test-specific CSV
                ...
                if should_run_benchmark:
                    if force_run:
                        logger.info("Force run enabled. Re-running VA Medium benchmark for all devices...")
                        modes_to_run = expected_modes
                    else:
                        logger.info(f"Running VA Medium benchmark for modes: {modes_to_run}")
                    ...
                    try:
                        ...
                        container_name = f"{configs.get('container_image', 'va_medium_bm')}_medium"
                        container_result = run_va_container(
                            docker_client=docker_client,
                            container_name=container_name,
                            image_name=configs.get("container_image", "va_medium_bm"),
                            image_tag=configs.get("image_tag", "1.0"),
                            benchmark_script="va_medium",  # Use medium benchmark script
                ...
                log_file_path = Path(va_results) / "va_medium_pipeline_runner.log"
    ...
    if results.metadata.get("status") == "N/A" and "failure_reason" in results.metadata:
        ...
        pytest.fail(f"VA Medium test failed - {failure_msg}")
