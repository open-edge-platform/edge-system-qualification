...
from sysagent.utils.system.drm import count_connected_displays
...
def get_connected_monitors() -> int:
    ...
    # Fallback: Check the DRM subsystem for physically connected displays
    try:
        return count_connected_displays()
    ...
    try:
        ...
        for device in devices:
            ...
            if "mass storage" in device_lower or "storage" in device_lower or "disk" in device_lower:
                storage_count += 1
            if (
                "audio" in device_lower
                or "sound" in device_lower
                or "speaker" in device_lower
                or "microphone" in device_lower
            ):
                audio_count += 1
...
def get_all_input_devices() -> Tuple[int, int, int, int, int, int]:
    ...
    try:
        ...
        for device in devices:
            ...
            if any(
                skip in name_line
                for skip in ["power button", "sleep button", "lid switch", "video bus", "hda intel", "intel hid"]
            ):
                continue

            # Detect keyboards
            is_keyboard = (
                "keyboard" in name_line or "kbd" in name_line or ("kbd" in handlers_line and "event" in handlers_line)
            )
            # Detect mouse
            is_mouse = (
                "mouse" in name_line
                or "pointing" in name_line
                or ("mouse" in handlers_line and "event" in handlers_line)
            )
...
def test_peripheral_detection(
    ...
    check_input_devices = any(
        [
            check_usb_keyboard,
            check_usb_mouse,
            check_ps2_keyboard,
            check_ps2_mouse,
            check_total_keyboard,
            check_total_mouse,
            check_usb_devices,
            check_usb,
        ]
    ...
    data_dir = os.path.join(core_data_dir, "data", "suites", "system", "peripheral")
    ...
    os.makedirs(peripheral_results_clean, mode=0o770, exist_ok=True)
    ensure_dir_permissions(peripheral_results_clean, uid=os.getuid(), gid=os.getgid(), mode=0o770)
    ...
    def _collect_peripheral_info():
        """Enumerate peripheral devices and build the result."""
        logger.info("Enumerating peripheral devices (USB, PS/2, network, graphics, monitors)")
        ...
        if check_usb or check_usb_controllers or check_usb_devices:
            ...
            logger.info(
                f"USB devices: {usb_devices_total} total (keyboard: {usb_kbd_simple}, mouse: {usb_mouse_simple}, storage: {usb_storage}, audio: {usb_audio}, camera: {usb_camera}, hub: {usb_hub})"
            )
            if usb_hub_details:
        ...
        if check_input_devices:
            ...
            logger.info(
                f"Input devices detected: {total_keyboard} keyboards (USB: {usb_keyboard}, PS/2: {ps2_keyboard}), {total_mouse} mouse (USB: {usb_mouse}, PS/2: {ps2_mouse})"
            )
            ...
            logger.info(
                f"Total USB peripheral devices: {usb_devices_total} (keyboard: {usb_keyboard}, mouse: {usb_mouse}, storage: {usb_storage}, audio: {usb_audio}, camera: {usb_camera}, hub: {usb_hub})"
            )
        ...
        # Step 5: Create result with message recorded in extended metadata
        return Result(
            name=f"{test_id} - {test_display_name}",
            metadata={"status": True},
            extended_metadata={
                "message": "Peripheral device detection completed",
                "peripheral_output_dir": peripheral_results,
            },
            metrics=metrics,
        )

    # Step 6: Execute enumeration with caching, surfacing interrupts/errors cleanly
    try:
        result = execute_test_with_cache(
            cached_result=cached_result,
            cache_result=cache_result,
            run_test_func=_collect_peripheral_info,
            test_name=test_name,
            configs=configs,
        )
    except KeyboardInterrupt:
        failure_message = "Interrupt detected during peripheral detection test execution"
        test_interrupted = True
        logger.error(failure_message)
    except Exception as e:
        test_failed = True
        failure_message = f"Unexpected error during peripheral detection test execution: {str(e)}"
        logger.error(failure_message, exc_info=True)

    # Ensure a result object always exists so the test terminates cleanly even
    # when interrupted before enumeration produced a result.
    if result is None:
        result = Result(
            name=f"{test_id} - {test_display_name}",
            ...
            extended_metadata={
                "peripheral_output_dir": peripheral_results,
                "message": failure_message or "Peripheral detection test did not complete",
            },
            metrics={},
        )

    # Step 7: Always validate and summarize so the result is recorded
    try:
        validate_test_results(
            results=result,
            configs=configs,
    ...
    # Step 8: Cache result — only when the test completed without interruption
    # or unexpected failure.  Caching an interrupted/failed placeholder result
    # would cause subsequent runs to find it in the cache and report a false
    # "passed" status without re-executing the test.
    if not test_interrupted and not test_failed:
        cache_result(result)

    logger.info(f"Peripheral device detection test completed: {test_display_name}")
