...
class HeadedVisualAIBenchmark(BaseProxyPipelineBenchmark):
    def __init__(
        self,
        name,
        device,
    ...
    def gen_gst_command(self, stream, resolution=None, codec=None, bitrate=None, model_name=None):
        ...
        compose_file_sink_cmd = f"h264parse ! mp4mux ! filesink location={self.sink_dir}/headed_visual_ai_composed_video.mp4 async=false sync=false "
        if device_type == "CPU":
        ...
        for i in range(stream):
            if device_type == "CPU":
                gst_cmd += f"filesrc location={video_src} ! decodebin "
            else:
            ...
            if device_type == "CPU":
                ...
                gst_cmd += f"! {self.post_proc_ele} scale-method=fast ! video/x-raw,width={cell_width},height={cell_height} ! multiqueue ! gvafpscounter starting-frame={starting_frame} ! comp.sink_{i} "

            if self.enable_mqtt:
                gst_cmd += f"t{i}. ! multiqueue ! gvametaconvert format=json json-indent=4 source={video_src} add-empty-results=true ! gvametapublish method=mqtt address={self.mqtt_address} mqtt-client-id=client{i} topic={self.mqtt_topic} "
            else:
                gst_cmd += f"t{i}. ! multiqueue ! gvametaconvert format=json json-indent=4 source={video_src} add-empty-results=true ! gvametapublish method=file file-path=/dev/null "
    ...
    def report_csv(self, *arg):
        tc_name = "Headed Visual AI Pipeline"
        compose_size = self.config["compose_size"]
        ...
        if not self.config_file_path:
            ...
            prefix = f"{tc_name}, {self.device}, H264 (4Mbps), 1080p@30, {model_name}, {self.monitor_num}, {result}"
            prefix_esc = (
                f"{tc_name}, {self.device}, H264 \\(4Mbps\\), 1080p@30, {model_name}, {self.monitor_num}, {result}"
            )
...
DETAIL_LOG_FILE_PREFIX = f"{OUTPUT_DIR}/headed_visual_ai_proxy_pipeline_runner"
RESULT_FILE_PREFIX = f"{OUTPUT_DIR}/headed_visual_ai_proxy_pipeline_runner"
TELEMETRY_FILE_PREFIX = f"{OUTPUT_DIR}/headed_visual_ai_proxy_pipeline_telemetry"
CSV_FILE_PREFIX = f"{OUTPUT_DIR}/headed_visual_ai_proxy_pipeline"
...
def run_headed_visual_ai_benchmark(device, monitor_num, is_mtl, has_igpu, config_file):
    ...
    headed_visual_ai_runner = HeadedVisualAIBenchmark(
        name="Headed Visual AI Benchmark",
        device=device,
    ...
    headed_visual_ai_runner.prepare()
...
if __name__ == "__main__":
    ...
    run_headed_visual_ai_benchmark(args.device, args.monitor_num, args.is_mtl, args.has_igpu, args.config_file)
