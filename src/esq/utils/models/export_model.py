...
def get_optimum_cli_path():
    ...
    raise ValueError(f"optimum-cli not found in the current environment. Python path: {python_path}, Python dir: {python_dir}")
...
def export_text_generation_model(
    ...
    if os.path.isfile(os.path.join(source_model, "openvino_model.xml")):
        ...
        if not os.path.isdir(llm_model_path) or overwrite_models:
            ...
            if task_parameters["extra_quantization_params"] is None:
                task_parameters["extra_quantization_params"] = ""
            # Use optimum-cli for model export
            optimum_cli_path = get_optimum_cli_path()
            logger.debug(f"optimum_cli_path: {optimum_cli_path}")
            ...
            logger.debug(f"optimum_command = {optimum_command}")
            if os.system(optimum_command):
                raise ValueError("Failed to export llm model", source_model)
        # Set relative path to the exported model directory
        model_path = "./"
    ...
    if os.path.isfile(os.path.join(source_model, "openvino_model.xml")):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if not os.path.isdir(embeddings_path) or overwrite_models:
                optimum_command = "{} export openvino --disable-convert-tokenizer --model {} --task feature-extraction --weight-format {} --trust-remote-code --library sentence_transformers {}".format(
                    get_optimum_cli_path(), source_model, precision, tmpdirname
                )
                if os.system(optimum_command):
            ...
            if not os.path.isdir(tokenizer_path) or overwrite_models:
                convert_tokenizer_command = "{} -m openvino_tokenizers.convert_tokenizer -o {} {} {}".format(
                    sys.executable, tmpdirname, source_model, set_max_context_length
                )
...
def export_rerank_model(
    ...
    if os.path.isfile(os.path.join(source_model, "openvino_model.xml")):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if not os.path.isdir(embeddings_path) or overwrite_models:
                optimum_command = "{} export openvino --disable-convert-tokenizer --model {} --task text-classification --weight-format {} --trust-remote-code {}".format(
                    get_optimum_cli_path(), source_model, precision, tmpdirname
                )
                if os.system(optimum_command):
