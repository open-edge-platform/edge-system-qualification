...
def get_optimum_cli_path():
    """Get the optimum-cli executable path in the same environment as Python."""
    python_path = sys.executable
    python_dir = os.path.dirname(python_path)

    # First, try to find optimum-cli in the same directory as python
    optimum_cli_path = os.path.join(python_dir, "optimum-cli")
    if os.path.exists(optimum_cli_path):
        logger.debug(f"Found optimum-cli at: {optimum_cli_path}")
        return optimum_cli_path

    # Second, try to find optimum-cli in PATH
    optimum_cli_in_path = shutil.which("optimum-cli")
    if optimum_cli_in_path:
        logger.debug(f"Found optimum-cli in PATH at: {optimum_cli_in_path}")
        return optimum_cli_in_path

    # If all else fails, raise an error
    raise ValueError(
        f"optimum-cli not found in the current environment. Python path: {python_path}, Python dir: {python_dir}"
    )
...
def add_common_arguments(parser):
    ...
    parser.add_argument(
        "--config_file_path", default="config.json", help="path to the config file", dest="config_file_path"
    )
    parser.add_argument(
        "--overwrite_models",
        default=False,
    ...
    parser.add_argument(
        "--target_device", default="CPU", help="CPU, GPU, NPU or HETERO, default is CPU", dest="target_device"
    )
    parser.add_argument(
        "--ov_cache_dir",
        default=None,
...
parser_text.add_argument(
    "--tool_parser",
    choices=["llama3", "phi4", "hermes3", "mistral", "qwen3coder", "gptoss", "devstral"],
...
parser_embeddings_ov = subparsers.add_parser(
    "embeddings_ov", help="export model for embeddings endpoint with directory structure aligned with OpenVINO tools"
)
add_common_arguments(parser_embeddings_ov)
parser_embeddings_ov.add_argument(
    "--skip_normalize", default=True, action="store_false", help="Skip normalize the embeddings.", dest="normalize"
)
parser_embeddings_ov.add_argument(
    "--pooling", default="CLS", choices=["CLS", "LAST", "MEAN"], help="Embeddings pooling mode", dest="pooling"
)
parser_embeddings_ov.add_argument(
    "--truncate",
    default=False,
...
parser_rerank_ov = subparsers.add_parser(
    "rerank_ov", help="export model for rerank endpoint with directory structure aligned with OpenVINO tools"
)
        ...
        if "mediapipe_config_list" in config_data:
            for mp_config in config_data["mediapipe_config_list"]:
                if "name" in mp_config and "base_path" in mp_config:
                    if not any(
                        d["config"]["name"] == mp_config["name"] + "_model" for d in config_data["model_config_list"]
                    ):
                        config_data["model_config_list"].append(
                            {"config": {"name": mp_config["name"] + "_model", "base_path": mp_config["base_path"]}}
                        )
...
def export_text_generation_model(
    ...
    if os.path.isfile(os.path.join(source_model, "openvino_model.xml")) or os.path.isfile(
        ...
        if not os.path.isdir(llm_model_path) or overwrite_models:
            ...
            print(f"  - Target device: {task_parameters['target_device']}")
            print(f"  - Precision: {precision}")
            print(f"  - Extra quantization params: {task_parameters['extra_quantization_params']}")
            print(f"  - Output path: {llm_model_path}")
            print("=" * 80)
    ...
    if draft_source_model:
        ...
        if os.path.isfile(os.path.join(draft_llm_model_path, "openvino_model.xml")):
            ...
            if not os.path.isdir(draft_llm_model_path) or overwrite_models:
                ...
                # Build draft model export command
                optimum_command = [
                    optimum_cli_path,
                    "export",
                    "openvino",
                ...
                # Add eagle3-specific options if enabled
                if task_parameters.get("draft_eagle3_mode", False):
                    print("Using eagle3 option for the draft model export")
                    optimum_command.extend(["--eagle3", "--task", "text-generation-with-past"])

                optimum_command.extend(
                    [
                        "--weight-format",
                        precision,
                        "--trust-remote-code",
    ...
    add_servable_to_config(
        config_file_path,
        model_name,
        os.path.relpath(os.path.join(model_repository_path, model_name), os.path.dirname(config_file_path)),
    )
...
def export_embeddings_model_ov(
    ...
    if not os.path.isdir(destination_path) or overwrite_models:
        # Get the optimum-cli path in the current environment
        optimum_cli_path = get_optimum_cli_path()
        logger.debug(f"Using optimum-cli from: {optimum_cli_path}")
...
def export_text2speech_model(
    ...
    if not os.path.isdir(destination_path) or overwrite_models:
        ...
        # Build model-kwargs JSON string
        model_kwargs = json.dumps({"vocoder": task_parameters["vocoder"]})
        ...
        if result.returncode != 0:
            raise ValueError(f"Failed to export text2speech model: {source_model}")

    # Enable autoescape for security compliance
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader, autoescape=True).from_string(t2s_graph_template)
    graph_content = gtemplate.render(model_path="./", **task_parameters)
    with open(os.path.join(model_repository_path, model_name, "graph.pbtxt"), "w") as f:
        f.write(graph_content)
    print("Created graph {}".format(os.path.join(model_repository_path, model_name, "graph.pbtxt")))
...
def export_speech2text_model(
    ...
    if not os.path.isdir(destination_path) or overwrite_models:
        ...
        result = run_command(optimum_command, timeout=export_timeout, capture_output=False)
        if result.returncode != 0:
            raise ValueError(f"Failed to export speech2text model: {source_model}")

    # Enable autoescape for security compliance
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader, autoescape=True).from_string(s2t_graph_template)
    graph_content = gtemplate.render(model_path="./", **task_parameters)
    with open(os.path.join(model_repository_path, model_name, "graph.pbtxt"), "w") as f:
        f.write(graph_content)
    print("Created graph {}".format(os.path.join(model_repository_path, model_name, "graph.pbtxt")))
...
def export_rerank_model_ov(
    ...
    # Enable autoescape for security compliance
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader, autoescape=True).from_string(rerank_graph_ov_template)
    graph_content = gtemplate.render(model_path="./", **task_parameters)
    with open(os.path.join(model_repository_path, model_name, "graph.pbtxt"), "w") as f:
        f.write(graph_content)
    print("Created graph {}".format(os.path.join(model_repository_path, model_name, "graph.pbtxt")))
    ...
    if os.path.isfile(os.path.join(model_name, "openvino_model.xml")):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if not os.path.isdir(tokenizer_path) or overwrite_models:
                ...
                shutil.move(
                    os.path.join(tmpdirname, "openvino_tokenizer.xml"), os.path.join(tokenizer_path, "model.xml")
                )
                shutil.move(
                    os.path.join(tmpdirname, "openvino_tokenizer.bin"), os.path.join(tokenizer_path, "model.bin")
                )
    # Enable autoescape for security compliance
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader, autoescape=True).from_string(rerank_graph_template)
    graph_content = gtemplate.render(model_name=model_name, **task_parameters)
    with open(os.path.join(model_repository_path, model_name, "graph.pbtxt"), "w") as f:
        f.write(graph_content)
    print("Created graph {}".format(os.path.join(model_repository_path, model_name, "graph.pbtxt")))
    # Enable autoescape for security compliance
    stemplate = jinja2.Environment(loader=jinja2.BaseLoader, autoescape=True).from_string(rerank_subconfig_template)
...
if __name__ == "__main__":
    # Parse CLI arguments only when script is executed directly
    args = vars(parser.parse_args())

    # CLI execution code - only runs when script is executed directly
