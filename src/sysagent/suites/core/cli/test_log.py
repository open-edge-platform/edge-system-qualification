...
def test_log(configs):
    ...
    with allure.step("Discover log files"):
        ...
        logger.debug(f"Found {len(log_files)} log files in {logs_dir} (excluding attach_logs and other internal files)")
        ...
        allure.attach(
            "\n".join(summary_lines), name="Log Files Discovery Summary", attachment_type=allure.attachment_type.TEXT
        )
    ...
    logger.debug(f"Completed CLI log attachment: {len(log_files)} files processed")
...
def _discover_log_files(logs_dir: str) -> List[str]:
    ...
    exclusion_patterns = ["*attach_logs*"]
...
def _attach_log_file_to_report(log_file_path: str) -> None:
    ...
    try:
        ...
        logger.debug(f"Attached log file: {file_name}")
