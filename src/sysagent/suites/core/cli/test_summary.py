...
def test_summary(configs):
    ...
    with allure.step("Discover summary files"):
        ...
        logger.debug(f"Found {len(summary_files)} summary files in {core_results_dir}")
        ...
        allure.attach(
            "\n".join(summary_lines), name="Summary Files Discovery Report", attachment_type=allure.attachment_type.TEXT
        )
    ...
    logger.debug(f"Completed CLI summary attachment: {len(summary_files)} files processed")
...
def _discover_summary_files(core_results_dir: str) -> List[str]:
    ...
    summary_patterns = ["test_summary_*.json", "*_summary.json"]
...
def _attach_summary_file_to_report(summary_file_path: str) -> None:
    ...
    try:
        ...
        logger.debug(f"Attached summary file: {file_name}")
