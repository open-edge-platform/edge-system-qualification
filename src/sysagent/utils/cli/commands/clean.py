...
def clean_data_dir(
    ...
    Args:
        ...
        clean_docker: Whether to remove Docker images built by this CLI (in addition to results)
        ...
        docker_only: Whether to remove only Docker images built by this CLI (skip results/logs cleaning)
    ...
    try:
        ...
        only_options = cache_only or thirdparty_only or data_only or venvs_only or docker_only
        cleaned_items = []
        ...
        if only_options:
            ...
            if docker_only:
                logger.info("Removing only ESQ-built Docker images")
                _clean_docker_images()
                cleaned_items.append("Docker images")
            ...
            if clean_all:
                ...
                clean_venvs = True
                clean_docker = True
                logger.info("Cleaning all directories (cache, thirdparty, data, venvs, and Docker images built by this CLI)")
...
def _clean_allure_reports_directory(data_dir: str):
    ...
    if os.path.exists(allure_reports_dir):
        ...
        try:
            ...
            logger.debug("Recreated empty allure reports directory")
        except Exception as e:
...
def _clean_thirdparty_directory(data_dir: str):
    ...
    if os.path.exists(thirdparty_dir):
        ...
        try:
            ...
            logger.debug("Recreated empty thirdparty directory")
        except Exception as e:
...
def _clean_application_data_directory(data_dir: str):
    ...
    if os.path.exists(app_data_dir):
        ...
        try:
            ...
            logger.debug("Recreated empty data directory")
        except Exception as e:
...
def _clean_docker_images():
    """Remove Docker images built by this CLI using label filter.

    Delegates to :meth:`DockerClient.remove_labeled_images` which filters
    by the ``group=<cli_name>`` label applied at build time.  Source/base
    images (FROM lines in Dockerfiles) are not labeled and will not be
    removed.
    """
    try:
        ...
        client.remove_labeled_images()
    except RuntimeError as e:
        logger.warning(f"Docker is not available - cannot clean Docker images: {e}")
