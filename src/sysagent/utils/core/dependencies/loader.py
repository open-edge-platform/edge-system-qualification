...
class DependencyConfigLoader:
    ...
    def load_all_package_configs(self) -> Dict[str, DependencyConfig]:
        ...
        for config_file in config_files:
            ...
            try:
                ...
                logger.debug(f"Loaded dependency config for package {package_name} from {config_file}")
    ...
    def load_package_config(self, package_name: str, config_path: Optional[str] = None) -> Optional[DependencyConfig]:
        ...
        try:
            ...
            config = self._parse_config(raw_config, package_name)
            self.loaded_configs[package_name] = config
            logger.debug(f"Loaded dependency config for package {package_name} from {config_path}")
            return config
    ...
    def merge_configs(self, package_names: Optional[List[str]] = None) -> DependencyConfig:
        ...
        logger.debug(f"Merged {len(merged_config.dependencies)} dependencies from {len(sorted_package_names)} packages")
    ...
    def _merge_installations(
        self, base_installation: DependencyInstallation, extension_installation: DependencyInstallation
    ) -> DependencyInstallation:
        """
        ...
        if (
            base_installation.os_type != extension_installation.os_type
            or base_installation.install_method != extension_installation.install_method
        ):
            logger.warning("Cannot merge installations with different OS types or methods")
